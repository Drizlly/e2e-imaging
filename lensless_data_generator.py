...
# smiley_img = Image.open("../../../../convolutions/Asset 3.png").convert("L")
...
class LenslessDataGenerator:
    ...
    def load_smiley_data(self, faces_bounds = [5, 10], scales_bounds = [0.8, 1.2], sensor_mask = None, offset = 0): # I ADDED THIS CODE!
        """ 
        Load and preprocess smiley dataset. Pads each image to be 32 x 32 
        
        Returns: 
        tuple: (x_train, x_test) converted to photon counts and subset of smiley data. 
        """
        (x_train, _), (x_test, _) = generate_distribution(10000, smiley_img, img_size = 96, faces = faces_bounds, scales = scales_bounds)
        ...
        if sensor_mask is not None:
            new_x_train = np.zeros((x_train.shape[0], 96, 96))  
            new_x_test = np.zeros((x_test.shape[0], 96, 96))
            sensor_mask_offset = np.ones((96,96), dtype=np.float32) * offset
            for i, img in enumerate(x_train):
                new_x_train[i] = (img * sensor_mask) + sensor_mask_offset          
            
            for i, img in enumerate(x_test):
                new_x_test[i] = (img * sensor_mask) + sensor_mask_offset
